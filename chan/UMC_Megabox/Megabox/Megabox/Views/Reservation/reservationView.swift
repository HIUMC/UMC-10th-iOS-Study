    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            .sheet(isPresented: $isSearchPresented) {
                MovieSearchView(movies: vm.movies) { movie in
    ...
    var body: some View {
        HStack(spacing: 10) {
            ForEach(Theater.allCases, id: \.self) { theater in
                Button(action: { vm.selectTheater(theater) }) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        LazyVGrid(columns: columns, spacing: 12) {
                            ForEach(auditorium.showtimes, id: \.start) { session in
                                if session.start == "11:30",
                                   let movie = vm.selectedMovie,
                                   let theater = vm.selectedTheater {
                                    NavigationLink {
                                        SeatSelectionView(
                                            movie: movie,
                                            theater: theater,
...
struct ShowtimeCell: View {
    let session: ShowtimeDTO
